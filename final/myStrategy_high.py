...
def computeReturnRate(dailyOhlcv, windowSize_1, windowSize_2, ceiling, floor):
    highVec = dailyOhlcv["high"].values
    capital=500000    # Initial available capital
    capitalOrig=capital  # original capital
    transFee = 100
    dataCount=len(highVec)             # day size
    ...
    for ic in range(dataCount):
        currentPrice=highVec[ic]   # current price
        # suggestedAction[ic]=myStrategy(highVec[0:ic], currentPrice, windowSize_1, alpha, beta)       # Obtain the suggested action
        suggestedAction[ic]=bestParamViaExhaustiveSearch.myStrategy_RSI(highVec[0:ic], currentPrice, windowSize_1, windowSize_2, ceiling, floor)       # Obtain the suggested action
...
def training(end_index):
    ...
    evalDays = 14
    highPricev = dailyOhlcv["high"].tail(evalDays).values
    clearPrice = dailyOhlcv.iloc[-3]["close"]
    beg_index = 0
    windowSizeMin=0; windowSizeMax=4;
    ceilingMin=50; ceilingMax=100
    floorMin=0; floorMax=50
    windowSizeRange = np.arange(5, 45, 5)

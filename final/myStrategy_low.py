...
def computeReturnRate(dailyOhlcv, windowSize_1, windowSize_2, ceiling, floor):
    lowVec = dailyOhlcv["low"].values
    capital=500000    # Initial available capital
    capitalOrig=capital  # original capital
    transFee = 100
    dataCount=len(lowVec)             # day size
    ...
    for ic in range(dataCount):
        currentPrice=lowVec[ic]   # current price
        # suggestedAction[ic]=myStrategy(lowVec[0:ic], currentPrice, windowSize_1, alpha, beta)       # Obtain the suggested action
        suggestedAction[ic]=bestParamViaExhaustiveSearch.myStrategy_RSI(lowVec[0:ic], currentPrice, windowSize_1, windowSize_2, ceiling, floor)       # Obtain the suggested action
...
def training(end_index):
    ...
    evalDays = 14
    lowPricev = dailyOhlcv["low"].tail(evalDays).values
    clearPrice = dailyOhlcv.iloc[-3]["close"]
    beg_index = 0
    windowSizeMin=0; windowSizeMax=4;   # 1 week to try
    ceilingMin=50; ceilingMax=100
    floorMin=0; floorMax=50
    windowSizeRange = np.arange(5, 45, 5)    # Two months
    windowSizeBest_1 = windowSizeBest_2 = floorBest = ceilingBest = returnRateBest = 0
    # Start exhaustive search
    for windowSize_1, windowSize_2 in zip(range(windowSizeMin+windowSizeRange[beg_index], windowSizeMax+windowSizeRange[beg_index]+1), range(windowSizeMin+windowSizeRange[end_index], windowSizeMax+windowSizeRange[end_index]+1)):
